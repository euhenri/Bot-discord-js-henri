...
module.exports = class extends Command {
    constructor(client) {
        super(client, {
            name: 'falar',
            description: 'Faz com que o bot fale uma mensagem.',
            options: [
                {
                    name: 'mensagem',
                    type: 'STRING',
    ...
    run = async (interaction) => {
        ...
        const channels = interaction.guild.channels.cache
            .filter(c => c.type === 'GUILD_TEXT' && c.permissionsFor(this.client.user.id).has (['SEND_MESSAGES', 'EMBED_LINKS']) && Command.permissionsFor(interaction.user.id).has('SEND_MESSAGES'))
        ...
        const reply = await interaction.reply({ 
            content: '**Selecione abaixo o canal onde deseja enviar mensagem**', 
        ...
        collector.on('collect', (i) => {
            ...
            canal.send({ embeds: [embed] })
                .the(() => interaction.editReply({ content: `Mensagem enviada com sucesso no canal ${canal.toString()}.`, components: [] }))
